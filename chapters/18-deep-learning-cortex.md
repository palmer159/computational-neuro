# Deep learning ↔ cortex: convergent representations

The empirical finding that brought deep learning back to neuroscience.

## The result that started this wave

📄 [Yamins, Hong, Cadieu, Solomon, Seibert & DiCarlo, 2014 — Performance-optimized hierarchical models predict neural responses in higher visual cortex](https://www.pnas.org/doi/10.1073/pnas.1403112111). Train a [CNN](https://en.wikipedia.org/wiki/Convolutional_neural_network) on ImageNet; its top layers predict macaque [IT](https://en.wikipedia.org/wiki/Inferior_temporal_gyrus) firing rates better than any prior model. ImageNet performance and IT-prediction are correlated.

> Yamins and colleagues trained convolutional neural networks to perform large-scale object recognition on ImageNet and then asked how well the resulting features predicted firing rates of neurons recorded from macaque inferotemporal (IT) cortex. They found that the top layers of high-performing CNNs predicted IT responses substantially better than any hand-engineered or biologically-motivated model that preceded them — and crucially, prediction quality scaled monotonically with the network's ImageNet performance. Lower CNN layers, in turn, best matched intermediate visual areas like V4, recovering the brain's ventral hierarchy without explicit constraints. The result vindicated a research strategy: optimizing for behavioral task performance on natural data automatically yields cortex-like representations, with no biological scaffolding required. It launched the modern wave of "task-driven" neural network models of cortex and motivated the Brain-Score benchmark that now serves as the standard model-vs-cortex leaderboard.

Three years later: [Kriegeskorte, 2015 — Deep neural networks: a new framework for modeling biological vision and brain information processing](https://doi.org/10.1146/annurev-vision-082114-035447). The framing that cemented the methodology.

## Brain-Score: the leaderboard

[Schrimpf et al. — Brain-Score: which artificial neural network for object recognition is most brain-like?](https://www.brain-score.org/). Standard benchmark: how well does your model predict [V1](https://en.wikipedia.org/wiki/Visual_cortex), [V2](https://en.wikipedia.org/wiki/Visual_cortex), [V4](https://en.wikipedia.org/wiki/Visual_cortex), IT recordings, plus behavioral consistency? Anyone publishing a model of vision references this.

Notable trend: the strongest predictors of brain data are **the strongest models on ImageNet/[CLIP](https://arxiv.org/abs/2103.00020)-style benchmarks**, not models that "look biological." Performance, not plausibility, predicts brain-likeness — within vision.

## Beyond vision: language

📄 [Schrimpf, Blank, Tuckute, Kauf, Hosseini, Kanwisher, Tenenbaum & Fedorenko, 2021 — The neural architecture of language: Integrative modeling converges on predictive processing](https://www.pnas.org/doi/10.1073/pnas.2105646118). LLMs predict cortical responses to sentences. The better the model at next-word prediction, the better at predicting the brain. **Language cortex looks like an autoregressive language model.**

> Schrimpf and colleagues evaluated 43 different language models against fMRI and ECoG recordings from humans hearing or reading sentences and found that next-token-prediction performance is the strongest predictor of how well a model matches language cortex. Autoregressive transformers (GPT family) outperformed bidirectional models (BERT) and earlier architectures (LSTMs) at predicting brain responses, with the relationship strengthening as models grew larger and more capable. The finding suggests that language cortex implements something computationally similar to next-token prediction — that the brain's language network and the LLM are solving overlapping problems with overlapping solutions. The result extends the Yamins et al. vision convergence into language and lays the empirical groundwork for treating LLMs as scientific models of the language network. It is a central piece of evidence in the broader claim that representational convergence between brains and trained networks is real, robust, and not specific to vision.

📄 [Caucheteux & King, 2022 — Brains and algorithms partially converge in natural language processing](https://www.nature.com/articles/s42003-022-03036-1). Confirms and extends.

📄 [Goldstein et al., 2022 — Shared computational principles for language processing in humans and deep language models](https://doi.org/10.1038/s41593-022-00766-3). [ECoG](https://en.wikipedia.org/wiki/Electrocorticography) recordings during natural conversation predicted by [GPT](https://en.wikipedia.org/wiki/Generative_pre-trained_transformer)-2 embeddings.

> Goldstein and colleagues recorded electrocorticographic (ECoG) activity directly from human cortex while participants engaged in spontaneous natural conversation — a much more ecologically valid task than the controlled-stimulus paradigms typical in language neuroscience. They found that GPT-2 contextualized word embeddings linearly predicted neural responses in the language network, and that the model's prediction accuracy at each time point reflected the brain's own predictive processing of upcoming words. The brain shows pre-onset prediction signals that mirror the LM's next-word probabilities, suggesting both systems are doing autoregressive prediction. The result extended Schrimpf et al. (2021) into naturalistic conversation and to direct cortical recordings, strengthening the convergence claim. It is one of the strongest empirical pieces of evidence that the language network and large language models implement computationally similar predictive processing.

**🤖 AI-relevance.** The convergence holds beyond vision. Language cortex behaves like a LM. This does not mean human language IS GPT — it means whatever cortex is computing for language is in a representational neighborhood that next-token prediction also reaches.

## Audition

📄 [Kell, Yamins, Shook, Norman-Haignere & McDermott, 2018 — A task-optimized neural network replicates human auditory behavior, predicts brain responses, and reveals a cortical processing hierarchy](https://mcdermottlab.mit.edu/papers/Kell_etal_2018_DNN_auditory_cortex.pdf).

CNNs trained on speech + music predict [A1](https://en.wikipedia.org/wiki/Auditory_cortex) and beyond.

## What this convergence is and is not

**Is.** Trained ANNs are by far the best **predictive models** of sensory cortex. They beat hand-crafted models that came before by a wide margin. This is non-trivial.

**Is not.** Mechanistic explanations. A CNN that predicts IT firing rates linearly does not tell us what learning rule cortex uses, what objective it optimizes, or what the actual circuit is doing. Predicting ≠ explaining.

This caveat has spawned a literature on **how to use ANNs as scientific models without confusing prediction for explanation**:

📄 [Cao & Yamins, 2024 — Explanatory models in neuroscience, Part 1 & 2](https://arxiv.org/abs/2104.01489) — careful philosophy.
📄 [Doerig et al., 2023 — The neuroconnectionist research programme](https://arxiv.org/abs/2209.03718) — the field's status doc.

## Representational Similarity Analysis ([RSA](https://www.frontiersin.org/articles/10.3389/neuro.06.004.2008/full))

A common methodology: compute pairwise stimulus-by-stimulus similarity matrices in (a) brain population activity, (b) [ANN](https://en.wikipedia.org/wiki/Artificial_neural_network) layer activations. Compare matrices.

📄 [Kriegeskorte, Mur & Bandettini, 2008 — Representational similarity analysis](https://www.frontiersin.org/articles/10.3389/neuro.06.004.2008/full).

> Kriegeskorte, Mur, and Bandettini introduced Representational Similarity Analysis (RSA), a methodology for comparing neural population codes — across brain regions, species, models, or experimental conditions — by reducing each representation to a stimulus-by-stimulus dissimilarity matrix. The key insight is that comparing these "representational dissimilarity matrices" sidesteps the need to align individual neurons or units across the systems being compared, which is impossible across species or between brains and ANNs. RSA enabled direct quantitative comparison between brain recordings and any computational model of the same stimuli, becoming the standard methodology for the deep-learning-vs-cortex literature that followed. The framework has since spread beyond neuroscience into mechanistic interpretability, where it is used to compare layers across architectures, fine-tuning regimes, and aligned models. It is the methodological bridge that makes brain-vs-ANN comparison rigorous rather than rhetorical.

**🤖 AI-relevance.** RSA is increasingly used in interpretability — comparing model layers across architectures, comparing aligned models, comparing pre/post fine-tuning. It is a scientific tool that came from neuro and is migrating into ML.

## What deep nets get wrong about cortex

Even within vision:

- **Adversarial robustness** is poor in CNNs but not in primates ([Geirhos et al.](https://arxiv.org/abs/1811.12231)).
- **Texture vs shape bias** differs.
- **Sample efficiency** is dramatically different — humans need orders of magnitude fewer examples.
- **Recurrence**. Cortex is massively recurrent; CNNs are mostly feedforward. Adding recurrence improves brain prediction, especially for difficult/occluded stimuli ([Kar et al., 2019](https://doi.org/10.1038/s41593-019-0392-5)).

## Strong takeaway

Where backprop-trained nets succeed at cortex prediction, they vindicate a research strategy: **objective + architecture + data ≈ representations**. They do not vindicate any specific account of how cortex itself learns.

## Sources

- [Doerig et al., 2023](https://arxiv.org/abs/2209.03718).
- [Yamins & DiCarlo, 2016](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6526887/).
- [Kriegeskorte & Douglas, 2018](https://doi.org/10.1038/s41593-018-0210-5).
