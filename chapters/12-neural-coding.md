...
📄 [Theunissen & Miller, 1995 — Temporal encoding in nervous systems](https://en.wikipedia.org/wiki/Temporal_coding). The criteria for "this is genuinely a temporal code, not just a fast rate code."

Examples:
- **Phase coding** in hippocampus: place cells fire at progressively earlier theta phases as the rat traverses the place field ("phase precession," [O'Keefe & Recce, 1993](https://en.wikipedia.org/wiki/Phase_precession)).
- **Latency coding** in retina, auditory: first-spike latency carries stimulus identity.
- **Synchrony** as a binding mechanism (controversial; see [Singer, 1999](https://en.wikipedia.org/wiki/Binding_problem)).
...
📄 [Pouget, Dayan & Zemel, 2000 — Information processing with population codes](https://en.wikipedia.org/wiki/Population_coding). Population codes naturally implement **probabilistic** representations — ideal for Bayesian inference (Ch 13).
...
**🤖 AI-relevance.** "Linear probes" in LLM interpretability ([Belinkov, 2022](https://aclanthology.org/2022.cl-1.7/)) are the same construct as population decoders in neuroscience. Both communities are converging on similar methods. The neuro side is older and has better priors on what decoding shows and doesn't.

## Sparse coding & efficient coding

📄 [Olshausen & Field, 1996](https://www.rctn.org/bruno/papers/sparse-coding.pdf) — Already cited in Ch 5. Imposing sparsity on a generative model of natural images yields V1-like Gabor receptive fields.

📄 [Barlow, 1961](https://en.wikipedia.org/wiki/Efficient_coding_hypothesis) — efficient-coding hypothesis: sensory cortex encodes natural inputs to maximize information per spike subject to metabolic cost.
...
- ML: β-VAE, diffusion latents, factor models.
...
- [Quian Quiroga & Panzeri, 2009 — Extracting information from neuronal populations](https://en.wikipedia.org/wiki/Neural_decoding).
