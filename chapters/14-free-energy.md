...
> Friston argues that any self-organizing system that resists dissipation must, on average, minimize the variational free energy of its sensory inputs — equivalently, minimize an upper bound on surprise. The principle unifies perception (updating internal beliefs to match observations), learning (updating the generative model), and action (changing the world to match predictions) under a single optimization objective. Mathematically, free energy reduces to the same variational quantity used in modern Bayesian deep learning, making the proposal at least operationally well-defined even where its scope is contested. The paper introduced "active inference," the extension that lets the brain minimize free energy by acting on the world, and it directly informs proposals like LeCun's autonomous-machine-intelligence architecture even when those proposals distance themselves from Friston's metaphysics. Critics argue the principle is too general to be falsifiable, but its practical algorithmic descendants — variational autoencoders, active-inference agents, predictive-coding hierarchies — have been productive.
...
> Friston and colleagues lay out active inference as a concrete process theory: agents minimize *expected* free energy over future trajectories of action and observation, where expected free energy decomposes neatly into an exploitation term (matching observations to prior preferences) and an exploration term (epistemic information gain). They show how perception, learning, planning, and exploration all reduce to a single objective under this decomposition, dissolving the boundaries between traditionally distinct cognitive processes. The paper provides the algorithmic recipes — message passing, variational updates, policy selection by softmax over expected free energy — that turn the principle into runnable agent code. Active inference agents have since been applied to navigation, decision-making, and robotic control as a unified alternative to standard reinforcement learning. The framework directly informs proposals like LeCun's autonomous-machine-intelligence and is one of the cleanest mathematical bridges between Bayesian inference, RL, and embodied agency.
