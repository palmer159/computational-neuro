...
📄 [Knill & Pouget, 2004 — The Bayesian brain: the role of uncertainty in neural coding and computation](https://en.wikipedia.org/wiki/Bayesian_approaches_to_brain_function). The field's manifesto.

## Cue integration: Bayesian to within ε

📄 [Ernst & Banks, 2002 — Humans integrate visual and haptic information in a statistically optimal fashion](https://en.wikipedia.org/wiki/Multimodal_integration#Maximum_likelihood_estimation). When subjects judge object size from sight + touch, they weight the two cues by their relative reliability — exactly the Bayesian-optimal rule. Strong empirical support that humans **really do** Bayesian inference at the perceptual level.

## Predictive coding: the candidate cortical algorithm

📄 [Rao & Ballard, 1999 — Predictive coding in the visual cortex: a functional interpretation of some extra-classical receptive field effects](https://en.wikipedia.org/wiki/Predictive_coding). The most-cited single computational theory of cortex.
...
- Many self-supervised learning objectives (BERT-style masked prediction, autoregressive next-token, JEPA) are computationally **predictive coding**, just at a larger scale and with backprop.
...
- Visual illusions explained by priors ([Adams, Graf & Ernst, 2004](https://en.wikipedia.org/wiki/Shape_from_shading)).
- Schizophrenia interpreted as miscalibrated priors ([Sterzer et al., 2018](https://en.wikipedia.org/wiki/Predictive_coding)).
- Place-cell and grid-cell activity as posterior over location.
- "End-stopping" and surround suppression in V1 explained quantitatively by predictive coding (Rao & Ballard).

## Probabilistic population codes

How does a population of spiking neurons represent a distribution? Two leading proposals:

- **Probabilistic population codes (PPCs)** — Poisson-like population activity directly encodes a likelihood; multiplication of two cues = sum of population activities. [Ma, Beck, Latham & Pouget, 2006](https://en.wikipedia.org/wiki/Population_coding).
...
**🤖 AI-relevance.** Both ideas have parallels in ML — variational inference (≈ PPC) and Monte Carlo / diffusion (≈ sampling code). The neuro choice between them is unresolved.
...
- [Spratling, 2017 — A review of predictive coding algorithms](https://en.wikipedia.org/wiki/Predictive_coding).
- [Clark, A., 2013 — Whatever next? Predictive brains, situated agents, and the future of cognitive science](https://en.wikipedia.org/wiki/Predictive_coding).
